...
1. **Serper** (Google search results as JSON): `"band name" chicago band` (skipped automatically if `SERPER_API_KEY` isn't set). When Serper finds the Instagram/Bandcamp link, it also captures the result's title and blurb — the site renders those under the link instead of a plain button. Links found via Bing/DDG don't carry a title/blurb, so they render as a plain link (never breaks, just plainer).
